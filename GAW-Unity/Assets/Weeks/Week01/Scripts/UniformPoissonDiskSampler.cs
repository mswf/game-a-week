    ...
    public static class UniformPoissonDiskSampler
    {
        ...
        static void AddFirstPoint(ref Settings settings, ref State state)
        {
            ...
            while (!added)
            {
                ...
                if (settings.RejectionSqDistance != null && MathS.Vector2DistanceSquared(settings.Center, p) > settings.RejectionSqDistance)
                    continue;
                added = true;
        ...
        static bool AddNextPoint(Vector2 point, ref Settings settings, ref State state)
	    {
            ...
            if (q.x >= settings.TopLeft.x && q.x < settings.LowerRight.x && 
                ...
                (settings.RejectionSqDistance == null || MathS.Vector2DistanceSquared(settings.Center, q) <= settings.RejectionSqDistance))
		    {
